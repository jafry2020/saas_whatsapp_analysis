...
import { Play, RotateCcw, Sparkles, Crown, Download } from 'lucide-react'
...
/** Top bar shown inside the analytics app (not the landing page). Hidden
 *  entirely when printing — see the print stylesheet in index.css. */
export function Navbar() {
  const { mode, reset, isPro, openUpgrade, openWrapped } = useApp()
  const [exportOpen, setExportOpen] = useState(false)
  return (
    <header className="sticky top-0 z-40 border-b border-line glass print:hidden">
      <div className="mx-auto max-w-[1400px] h-16 px-4 md:px-6 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => setExportOpen(true)} className="hidden sm:inline-flex">
      ...
      <div className="md:hidden border-t border-line px-4 py-2 flex items-center justify-center gap-2">
        <ModeToggle size="sm" />
        <Button variant="outline" size="sm" onClick={() => setExportOpen(true)}><Download size={14} /> Export</Button>
      </div>
